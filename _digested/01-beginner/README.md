...
Beginner 文档和其他主题文档有三点显著差异（详见 `../WRITING-STYLE.md` 第五节）：

1. **开篇 callout 堆叠** — 正文前通常有 2-3 个 `> [!TIP]` / `> [!IMPORTANT]` / `> [!NOTE]` 叠放
2. **术语对照更严格** — 每次出现专有术语都应标注「中文（English）」，不只在首次
3. **止步提示** — 文末应有 "停在这里也已经够用"，告诉读者不必深挖到实现层
4. **验证检查表** — 至少一张 3 列表格（检查项 / 命令 / 成功信号）确认读者能动手操作
5. **1 分钟诊断卡** — 一张快速分流表（现象 → 先看哪里）
